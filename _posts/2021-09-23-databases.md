...
Notice again that column-oriented datastore are totally different from he wide-column stores we talked about above. 
...
Neo4j seems to be the popular database that has “nattive” graph storage. I guess that means that the database itself was built to be optimal for querying with the graphQL syntax. 
